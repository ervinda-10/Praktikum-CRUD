...
app.get("/pegawai", (req,res) => {
    let sql = "select * from tb_pegawai"
...
app.post("/pegawai", (req,res) => {
    ...
    let sql = "select * from tb_pegawai where id_pegawai like '%"+find+"%' or nama_pegawai like '%"+find+"%' or alamat_pegawai like '%"+find+"%'"
    db.query(sql, (err, result) => {
        if (err) {
            throw err
        } else {
...
app.post("/pegawai/save", (req,res) => {
    ...
    let sql = "insert into tb_pegawai set ?"
...
app.post("/pegawai/update", (req,res) => {
    let data = [{
        id_pegawai: req.body.nip,
        nama_pegawai: req.body.nama_pegawai,
        alamat_pegawai: req.body.alamat_pegawai
    }, req.body.id_pegawai]
    let message = ""

    let sql = "update tb_pegawai set ? where id_pegawai = ?"
    db.query(sql, data, (err,result) => {
        if (err) {
            message = err.message
        } else {
...
app.delete("/pegawai/:id_pegawai", (req,res) => {
    ...
    let message = ""
    let sql = "delete from tb_pegawai where ?"
    db.query(sql, data, (err,result) => {
        if (err) {
            message = err.message
        } else {
